...
type CConst*[T] {.importcpp: "std::add_const<'0>::type", header: "<type_traits>", cppNonPod.} = object
  ...
  ##
  ## A variable whose type is in the form `CConst[T]`, where `T` can be any type, will (probably) cause an error at the C++ compile-time. (But it can be of the type `ptr CConst[T]`.)
  ##
  ## **Example:**
  ##
  ## .. code-block::
